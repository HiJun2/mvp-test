...
function ImageManager({ title, image, busy, onUpload, onDeactivate }: { title: string; image: AdminImage; busy: boolean; onUpload: (file: File, description: string) => void; onDeactivate: (id: string) => void }) {
  return <div className={styles.imageManager}><div className={styles.imagePreview}>{image ? <img src={image.imageUrl} alt={image.description || title} /> : <span><ImagePlus />등록된 이미지가 없어요</span>}</div><div className={styles.imageControls}><div><strong>{title}</strong>{image && <small>버전 {image.version}{image.createdAt ? ` · ${new Intl.DateTimeFormat("ko-KR", { dateStyle: "medium" }).format(new Date(image.createdAt))}` : ""}</small>}</div><div><label className={styles.uploadButton}><Upload />{busy ? "처리 중" : image ? "이미지 교체" : "이미지 등록"}<input type="file" accept="image/png,image/jpeg,image/webp,image/avif" disabled={busy} onChange={(event) => { const file = event.target.files?.[0]; if (file) onUpload(file, image?.description || title); event.target.value = ""; }} /></label>{image && <button className={styles.deactivateButton} disabled={busy} onClick={() => onDeactivate(image.id)}><Trash2 />비활성화</button>}</div></div></div>;
}
